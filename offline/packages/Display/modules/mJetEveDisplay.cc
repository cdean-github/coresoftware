...
mJetEveDisplay::mJetEveDisplay(boost::shared_ptr<PHEveDisplay> dispin)
    : mPHEveModuleBase(), _evedisp(dispin), _vtxmap(NULL), _jetmap(NULL),
      _reco_jets(NULL), radius(0.3), length(300) {
...
void mJetEveDisplay::init(PHCompositeNode *topNode) {}

void mJetEveDisplay::init_run(PHCompositeNode *topNode) {}

bool mJetEveDisplay::event(PHCompositeNode *topNode) {
  clear();
  if (verbosity)
    std::cout << "mJetEveDisplay - event() begins." << std::endl;
  try {
...
void mJetEveDisplay::end(PHCompositeNode *topNode) {}

void mJetEveDisplay::create_nodes(PHCompositeNode *topNode) {
  _vtxmap = findNode::getClass<SvtxVertexMap>(topNode, "SvtxVertexMap");
  if (_vtxmap->empty())
    std::cout << "SvtxVertexMap is empty!!! " << std::endl;
...
void mJetEveDisplay::clear() { _reco_jets->DestroyElements(); }
