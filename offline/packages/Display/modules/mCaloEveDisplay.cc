...
mCaloEveDisplay::mCaloEveDisplay(boost::shared_ptr<PHEveDisplay> dispin)
    : mPHEveModuleBase(), _evedisp(dispin), _cemc_list(NULL),
      _hcalin_list(NULL), _hcalout_list(NULL),
      _pal(new TEveRGBAPalette(0, 130)), _cemc_clusters(NULL),
      _hcalin_clusters(NULL), _hcalout_clusters(NULL), _cemc_towers(NULL),
      _hcalin_towers(NULL), _hcalout_towers(NULL), _cemc_towergeo(NULL),
      _hcalin_towergeo(NULL), _hcalout_towergeo(NULL) {
...
void mCaloEveDisplay::init(PHCompositeNode *topNode) {}

void mCaloEveDisplay::init_run(PHCompositeNode *topNode) {}

bool mCaloEveDisplay::event(PHCompositeNode *topNode) {
  if (verbosity)
    std::cout << "mCaloEveDisplay - event() begins." << std::endl;
  clear();
  try {
    ...
    if (first)
      std::cout << "mCaloEveDisplay::event Exception: " << e.what()
                << std::endl;
    first = false;
  }
...
void mCaloEveDisplay::end(PHCompositeNode *topNode) {}

void mCaloEveDisplay::create_nodes(PHCompositeNode *topNode) {
  _cemc_clusters =
      findNode::getClass<RawClusterContainer>(topNode, "CLUSTER_CEMC");
  _cemc_towers =
      findNode::getClass<RawTowerContainer>(topNode, "TOWER_CALIB_CEMC");
  _cemc_towergeo =
      findNode::getClass<RawTowerGeomContainerv1>(topNode, "TOWERGEOM_CEMC");
  ...
  _hcalin_towergeo =
      findNode::getClass<RawTowerGeomContainerv1>(topNode, "TOWERGEOM_HCALIN");
  ...
  _hcalout_towergeo =
      findNode::getClass<RawTowerGeomContainerv1>(topNode, "TOWERGEOM_HCALOUT");
...
void mCaloEveDisplay::draw_clusters(bool is_cemc_on, bool is_hcalin_on,
                                    bool is_hcalout_on) {
  // box set approach
      ...
      for (RawCluster::TowerConstIterator iter = begin_end.first;
        ...
        if (verbosity > 2)
          std::cout << "tower " << iter->first << ": energy " << energy
                    << ", phi " << phi << ", center x " << center_x
                    << ", size x " << size_x << ", center y " << center_y
                    << ", size y " << size_y << ", center z " << center_z
                    << ", size z " << size_z << std::endl;
        size = 4.0;
        scale = TMath::Log(energy) + 4; // 1MeV :1, 1GeV :4
        if (fabs(center_x) < 0.0001)
          phi = atan2(center_y, 0.0001);
        else
          phi = atan2(center_y, center_x);
        for (int i = 0; i < 2; i++) {
          for (int j = 0; j < 2; j++) {
            if (!i)
              vtx.Set(pow(-1, i) * size / 2., pow(-1, j) * size / 2.);
            else
              vtx.Set(pow(-1, i) * size / 2., pow(-1, j + 1) * size / 2.);
            vtx = vtx.Rotate(phi);
