...
mG4EveDisplay::mG4EveDisplay(boost::shared_ptr<PHEveDisplay> dispin)
    : mPHEveModuleBase(), _evedisp(dispin),
      _truth(NULL), // truth info container
      _svtxevalstack(NULL), _jetmap(NULL), _prop(NULL), _true_tracks(NULL),
      _true_jets(NULL), radius(0.3), length(300) {
...
void mG4EveDisplay::init(PHCompositeNode *topNode) {}

void mG4EveDisplay::init_run(PHCompositeNode *topNode) {}

bool mG4EveDisplay::event(PHCompositeNode *topNode) {

  if (verbosity)
    std::cout << "mG4EveDisplay - event() begins." << std::endl;
  clear();
  try {
...
void mG4EveDisplay::end(PHCompositeNode *topNode) {}

void mG4EveDisplay::create_nodes(PHCompositeNode *topNode) {
  _truth = findNode::getClass<PHG4TruthInfoContainer>(topNode, "G4TruthInfo");
  if (!_truth)
    std::cout << "TruthInfoContainer node not found!! " << std::endl;
  if (!_svtxevalstack) {
  ...
  if (!_jetmap)
    std::cout << "JetMap node not found!! " << std::endl;
  if (verbosity)
    std::cout << "mG4EveDisplay - nodes created." << std::endl;
}

void mG4EveDisplay::draw_ideal_tracks() {
  if (verbosity > 1)
    std::cout << "mG4EveDisplay - draw_ideal_tracks() begins. " << std::endl;
  ...
  for (PHG4TruthInfoContainer::ConstIterator iter = range.first;
    ...
    trk_reco->fP.Set(particle->get_px(), particle->get_py(),
                     particle->get_pz());
    int pid = particle->get_pid();
    int tid = particle->get_track_id();
    if (verbosity > 2)
      std::cout << "mG4EveDisplay - particle id : " << pid
                << ", track id : " << tid << std::endl;
    if (fabs(pid) == 211 || fabs(pid) == 321 || fabs(pid) == 11 ||
        fabs(pid) == 13 || fabs(pid) == 15 || fabs(pid) == 17) {
      if (pid > 0)
        trk_reco->fSign = 1;
      else
        trk_reco->fSign = -1;
...
void mG4EveDisplay::draw_g4_tracks() {
               ...
               for (SvtxTrack::ConstClusterIter iter = track->begin_clusters();
                      ...
                      //unsigned int layer = cluster->get_layer();
                                             //float cluste = cluster->get_e();
                                                                    //if(e <
     0.1) continue; float posx = cluster->get_position(0); float posy =
     cluster->get_position(1); float posz = cluster->get_position(2);
                      if(verbosity>2) std::cout<<"posx= "<<posx <<", posy=
     "<<posy<<", posz= "<<posz<<std::endl;
                      trk->AddPathMark(TEvePathMarkD(TEvePathMarkD::kDaughter,
                                       TEveVectorD(posx, posy, posz)));
                      trk->SetRnrPoints(kTRUE);
...
void mG4EveDisplay::draw_jets() {
  ...
  for (JetMap::Iter iter = _jetmap->begin(); iter != _jetmap->end(); ++iter) {
    ...
    if (pt < pt_threshold)
      continue;
    if (verbosity > 2)
      std::cout << "Jet " << id << " : eta " << eta << ", phi " << phi
                << ", pt " << pt << std::endl;
