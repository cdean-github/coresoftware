...
#include <trackbase_historic/SvtxVertexMap.h> // for maps & tpc
...
mSvtxEveDisplay::mSvtxEveDisplay(boost::shared_ptr<PHEveDisplay> dispin)
    : mPHEveModuleBase(), _evedisp(dispin), _prop(NULL), _svtx_tracks(NULL) {
...
void mSvtxEveDisplay::init(PHCompositeNode *topNode) {}

void mSvtxEveDisplay::init_run(PHCompositeNode *topNode) {}

bool mSvtxEveDisplay::event(PHCompositeNode *topNode) {
  clear();

  if (verbosity)
    std::cout << "mSvtxEveDisplay - event() begins." << std::endl;
  try {
    ...
    if (first)
      std::cout << "mSvtxEveDisplay::event Exception: " << e.what()
                << std::endl;
    first = false;
  }
...
void mSvtxEveDisplay::end(PHCompositeNode *topNode) {}

void mSvtxEveDisplay::create_nodes(PHCompositeNode *topNode) {
  _vertexmap = findNode::getClass<SvtxVertexMap>(topNode, "SvtxVertexMap");
  if (!_vertexmap)
    std::cout << "SvtxVertexMap node not found!!" << std::endl;
  _trackmap = findNode::getClass<SvtxTrackMap>(topNode, "SvtxTrackMap");
  if (!_trackmap)
    std::cout << "SvtxTrackMap node not found!!" << std::endl;
  _clustermap = findNode::getClass<TrkrClusterContainer>(topNode, "TRKR_CLUSTER");
  if (!_clustermap)
    std::cout << "SvtxClusterMap node not found!!" << std::endl;
  if (verbosity)
    std::cout << "mSvtxEveDisplay - nodes created." << std::endl;
}

void mSvtxEveDisplay::draw_tracks() {
  // svtx tracks
  for (SvtxVertexMap::Iter iter = _vertexmap->begin();
    ...
    for (SvtxVertex::TrackIter iter = vertex->begin_tracks();
      ...
      for (SvtxTrack::ConstClusterIter iter = track->begin_clusters();
        ...
        // if(e < 0.1) continue;
        float posx = cluster->getX();
        float posy = cluster->getY();
        float posz = cluster->getZ();
        if (verbosity > 2)
...
bool mSvtxEveDisplay::pid_cut(int pid) {
