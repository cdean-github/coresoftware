...
PHEveDisplay::PHEveDisplay(int w, int h, bool use_fieldmap, bool use_geofile,
                           const std::string &mapname,
                           const std::string &filename, int verb)
    : _top_list(NULL), _svtx_list(NULL), _calo_list(NULL), _jet_list(NULL),
      _true_list(NULL), cnt_prop(NULL), mapped_field(NULL), _width(w),
      _height(h), _use_fieldmap(use_fieldmap), _use_geofile(use_geofile),
      _jet_pt_threshold(5.0), _calo_e_threshold(0.2), map_filename(mapname),
      geo_filename(filename), verbosity(verb) {
  if (verbosity)
    std::cout << "PHEveDisplay initialized. " << std::endl;
...
PHEveDisplay::~PHEveDisplay() {
  try {
    ...
    std::cout << "Exception caught during deconstruction: " << e.what()
              << std::endl;
  }
}

void PHEveDisplay::load_geometry(PHCompositeNode *topNode, TEveManager *geve) {
  ...
  if (strcmp(geo_filename.c_str(), "sphenix_mie_geo.root") == 0 && _use_geofile)
    det_config = 1;
  else if (strcmp(geo_filename.c_str(), "sphenix_maps+tpc_geo.root") == 0 &&
           _use_geofile)
    det_config = 2;
  for (int i = 0; i < nd; i++) {
    ...
    if (name.find("InnerHcal") < 5 ||
        name.find("OuterHcal") < 5) { // make hcal transparent
...
void PHEveDisplay::config_bfields() {
  if (_use_fieldmap) {
    if (verbosity > 1)
      std::cout << "PHEveDisplay::config_bfields:"
                << "Using mapped magnetic fields for track propagation"
                << std::endl;
    cnt_prop = new TEveTrackPropagator("cnt_prop", "Central Field Propagator",
...
void PHEveDisplay::go_fullscreen(TEveManager *geve) {
  ...
  TEveCompositeFrameInPack *packframe =
      dynamic_cast<TEveCompositeFrameInPack *>(fEveFrame);
  if (packframe) {
  ...
  TEveCompositeFrameInMainFrame *slot =
  ...
  mf->AddFrame(
      slot, new TGLayoutHints(kLHintsNormal | kLHintsExpandX | kLHintsExpandY));
  slot->MapWindow();
...
PHEveDisplay::MappedField::MappedField(const std::string &fname)
    : TEveMagField(),
      ...
      _fieldmap((PHField*) new PHField3DCartesian(fname, -1.4/1.5)) {}

TEveVectorD PHEveDisplay::MappedField::GetFieldD(Double_t x, Double_t y,
                                                 Double_t z) const {
  double loc[3];
  loc[0] = x;
  loc[1] = y;
  ...
  _fieldmap->GetFieldValue(&loc[0], &bvec[0]);
  TEveVectorD vec(bvec[0], bvec[1],
