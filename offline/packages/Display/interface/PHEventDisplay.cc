...
PHEventDisplay::PHEventDisplay(int w, int h,
                               bool _use_fieldmap = true,
                               bool _use_geofile = true,
                               const std::string &_mapname = "sPHENIX.2d.root",
    ...
    , nevent(0)
{
  ...
  if (verbosity)
    std::cout << "PHEventDisplay instantiated." << std::endl;
}
...
int PHEventDisplay::Init(PHCompositeNode *topNode) {
  ...
  std::cout << "svtx :" << is_svtx_on << ", cemc : " << is_cemc_on
            << ", hcalin : " << is_hcalin_on << ", hcalout : " << is_hcalout_on
            << ", jet : " << is_jet_on << ", truth : " << is_truth_on
...
int PHEventDisplay::InitRun(PHCompositeNode *topNode) {
  ...
  try {
    ...
    if (is_truth_on) {
      ...
      if (verbosity)
        std::cout << "PHEventDisplay - mG4EveDisplay module registered."
                  << std::endl;
    }
    if (is_svtx_on) {
      register_module<mSvtxEveDisplay>();
      if (verbosity)
        std::cout << "PHEventDisplay - mSvtxEveDisplay module registered."
                  << std::endl;
    }
    if (is_cemc_on || is_hcalin_on || is_hcalout_on) {
      register_module<mCaloEveDisplay>();
      if (verbosity)
        std::cout << "PHEventDisplay - mCaloEveDisplay module registered."
                  << std::endl;
    }
    if (is_jet_on) {
      register_module<mJetEveDisplay>();
      if (verbosity)
        std::cout << "PHEventDisplay - mJetEveDisplay module registered."
                  << std::endl;
    }
    ...
    std::cout << "Exception caught while initializing sPHENIX event display: "
              << e.what() << std::endl;
  }
...
int PHEventDisplay::process_event(PHCompositeNode *topNode) {
  ...
  std::for_each(_modules.begin(), _modules.end(),
                bind(&mPHEveModuleBase::event, _1, topNode));
  return 0;
}

int PHEventDisplay::End(PHCompositeNode *topNode) { return 0; }

void PHEventDisplay::update_scene() {
  if (verbosity)
    std::cout << "PHEventDisplay - update_scene() nevent = " << nevent
              << std::endl;
...
void PHEventDisplay::run_evt_in_thread() {
  if (verbosity)
    std::cout << "PHEventDisplay - run_evt_in_thread() nevent = " << nevent
              << std::endl;
  if (!pthread_mutex_trylock(&_mutex)) {
    ...
    _pending_update = false;
    _update_thread = boost::make_shared<boost::thread>(
        bind(&PHEventDisplay::reco_thread, this));
  }
}

void PHEventDisplay::reco_thread() {
  if (verbosity)
    std::cout << "PHEventDisplay - reco_thread() nevent = " << nevent
              << std::endl;
  nevent++;
  Fun4AllServer *se = Fun4AllServer::instance();
  se->run(1);
  if (verbosity > 1)
    std::cout << "reco_thread() run(1) complete." << std::endl;
  _pending_update = true;
  if (verbosity > 1)
    std::cout << "reco_thread() pendinding update " << std::endl;
  pthread_mutex_unlock(&_mutex);
  if (verbosity > 1)
    std::cout << "reco_thread() mutex unlock()" << std::endl;
}

void PHEventDisplay::draw_default() {
  if (verbosity)
    std::cout << "PHEventDisplay - draw_default() begins." << std::endl;
  if (verbosity > 1)
    std::cout << "draw_default() FullRedraw3D." << std::endl;
  gEve->FullRedraw3D(kTRUE);
  if (verbosity > 1)
    std::cout << "draw_default() TGLViewer." << std::endl;
  TGLViewer *v = gEve->GetDefaultGLViewer();
  //  v->ColorSet().Background().SetColor(kMagenta+4);
  v->SetGuideState(TGLUtil::kAxesOrigin, kTRUE, kFALSE, 0);
  v->RefreshPadEditor(v);
...
void PHEventDisplay::go_fullscreen() { _PHEveDisplay->go_fullscreen(gEve); }
